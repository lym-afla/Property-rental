...
two currencies, walking an undirected graph of the FX table's per-pair
columns (today: ``EURUSD``, ``GBPUSD``, ``USDRUB``) via networkx
Bellman-Ford shortest-path. It is the conversion engine underneath
``Transaction.financials`` and ``Tenant.rent_total`` and is the thing
Task 9 (wide->long schema migration) and Task 10 (FX cache) must NOT
change observable behavior for.
...
# FX graph builder
# ---------------------------------------------------------------------------
# Today's wide FX schema has exactly three pair columns — EURUSD, GBPUSD,
# USDRUB — so the graph is fixed: {EUR-USD, GBP-USD, USD-RUB}. We seed one
# row per pair on the same date. The specific rate values are chosen to be
# easy to eyeball when tracing the algorithm.
#
# Graph:
...
def build_fx_graph():
    ...
    """
    FXFactory(date=date(2024, 1, 10), EURUSD=Decimal("1.10"))
    FXFactory(date=date(2024, 1, 10), GBPUSD=Decimal("1.25"))
    FXFactory(date=date(2024, 1, 10), USDRUB=Decimal("90.00"))
    return _AS_OF


# ---------------------------------------------------------------------------
# Direct pair: EUR -> USD (single hop, column EURUSD, source-first)
# ---------------------------------------------------------------------------

def test_get_rate_direct(db):
    as_of = build_fx_graph()
    result = FX.get_rate("EUR", "USD", as_of)
    # PINNED QUIRK: stored EURUSD=1.10, but get_rate returns the
...
def test_get_rate_two_hop(db):
    ...
    # Path is EUR -> USD -> RUB. Both hops are source-first in their
    # column name, so both multiply: 1 * 1.10 * 90.00 = 99.00. Tail
    # inversion: round(1/99, 6) = 0.010101.
...
def test_get_rate_no_path(db):
    as_of = build_fx_graph()
    # JPY has no column in today's schema, so it is not a node in the
    ...
    # if Task 9/10 wraps this in a ValueError or returns None instead,
