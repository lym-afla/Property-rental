...
import { cashTable, chartState, compactPeriod, seriesWithVisualTokens, type ChartDataProps } from './chartUtils'

const TREND_KEYS = new Set(['total_income', 'total_expenses'])

export function RevenueExpenseTrendChart({ data, isLoading, isError, onRetry }: ChartDataProps) {
  const [hiddenKeys, setHiddenKeys] = useState<Set<string>>(new Set())
  const state = chartState('Revenue and expenses', { data, isLoading, isError, onRetry })
  const series = data ? seriesWithVisualTokens(data.series.filter((item) => TREND_KEYS.has(item.key))) : []
  const visibleSeries = series.filter((item) => !hiddenKeys.has(item.key))
  return (
    <AnalyticsChartCard state={state} title="Revenue and expenses" subtitle="Server-provided total income and total expenses by period." controls={data && <ChartLegend series={series} hiddenKeys={hiddenKeys} onToggle={(key) => setHiddenKeys((current) => { const next = new Set(current); next.has(key) ? next.delete(key) : next.add(key); return next })} />} table={data ? cashTable(data, series) : undefined}>
      {data && <ResponsiveContainer width="100%" height="100%"><LineChart data={data.points} margin={{ top: 16, right: 12, left: 4, bottom: 4 }}>
