...
test('populated dashboard visual baseline preserves chart layout and exact values at each required viewport', async ({ page }) => {
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.includes('/analytics/portfolio/currency-exposure/')) return json({ metric: 'currency_exposure', grain: 'month', currency: 'USD', scale: 1, ...range, measure: 'property_value', measure_label: 'Property value', series: [{ key: 'eur', label: 'EUR', kind: 'native_currency' }, { key: 'usd', label: 'USD', kind: 'native_currency' }], points: [{ ...january, eur: 250000, usd: 150000 }, { ...february, eur: 255000, usd: 152000 }], coverage: [{ ...january, currency: 'USD', status: 'ok', missing_count: 0, stale_count: 0 }, { ...february, currency: 'USD', status: 'ok', missing_count: 0, stale_count: 0 }] })
