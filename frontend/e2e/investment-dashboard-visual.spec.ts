...
test('populated dashboard visual baseline preserves chart layout and exact values at each required viewport', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path.includes('/analytics/portfolio/profit-loss/')) {
      const columns = [...Array.from({ length: 21 }, (_, index) => { const year = String(2004 + index); return { key: year, label: year, start: `${year}-01-01`, end: year === '2024' ? range.end : `${year}-12-31` } }), { key: 'ytd', label: 'YTD', start: '2024-01-01', end: range.end }]
      const positives = Object.fromEntries(columns.map(({ key }, index) => [key, key === 'ytd' ? 1650 : 1000 + index * 50]))
