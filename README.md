...
+ The application is multi-currency. Each property has its own _natural_ currency. Data representation can be shown in a user-selected reporting currency, and the home page summarizes portfolio statistics in the default currency. In production, FX acquisition is performed by the scheduled `refresh_fx` management command, which scans dated rental records for missing historical currency pairs instead of fetching rates for an arbitrary calendar day; ordinary web requests and financial writes do not call external FX providers.
